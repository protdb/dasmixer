"""Samples section - expandable panels per sample with status indicators and actions."""
...
class SamplesSection(BaseSection):
    """
    Section showing samples as expandable panels (ft.ExpansionPanelList).

    Loading strategy:
    - On project open: show cached stats instantly (fast), then refresh all in background.
    - Manual "Update" button: recalculates all stats, writes to cache.
    - Per-sample actions: refresh only that sample's panel.

    Each panel header: status marker, outlier flag, counters.
    Each panel body: file tree, additions, action buttons.
    """
    ...
    def __init__(self, project: Project, state: SamplesTabState, parent_tab):
        ...
        # Maps sample_id → index in _panels_list.controls
        self._panel_index: dict[int, int] = {}
        # Current sample list (preserved across refreshes)
    ...
    def _build_content(self) -> ft.Control:
        ...
        self._update_btn = ft.ElevatedButton(
            ...
            on_click=lambda e: self.page.run_task(self._on_update_clicked)
    ...
    # ------------------------------------------------------------------
    # Public load_data — called on project open
    # ------------------------------------------------------------------
    ...
    async def load_data(self):
        ...
        # Bulk-fetch all cached stats in one query
        ...
        # Background-refresh uncached samples (silently)
        if self._samples:
            uncached_ids = [
                int(s.id)
    ...
    # ------------------------------------------------------------------
    # Manual "Update" button handler — full recalc with loader
    # ------------------------------------------------------------------

    async def _on_update_clicked(self):
        """Recalculate and cache stats for ALL samples, show loader."""
    ...
    # ------------------------------------------------------------------
    # Per-sample panel refresh (called after per-sample actions)
    # ------------------------------------------------------------------

    async def refresh_single_panel(self, sample_id: int) -> None:
        """
        Recompute stats for one sample, update cache, and redraw only its panel.

        Called after per-sample actions (Calculate ions, Delete file, etc.).
        Does NOT rebuild the entire list.
        """
        # Find sample object
        sample = next((s for s in self._samples if s.id == sample_id), None)
        if sample is None:
            # Sample was deleted or not in list — do full reload
            await self.load_data()
            return

        # Refresh sample object (e.g. outlier flag may have changed)
    ...
    async def _refresh_single_stats_in_place(
        self, sample_id: int, save_cache: bool = False
    ) -> None:
        """
        Compute fresh stats for sample_id, optionally cache, and update its panel in the list.
        """
        # Find sample
        sample = next((s for s in self._samples if s.id == sample_id), None)
        if sample is None:
        ...
        min_proteins, min_idents = self._thresholds()
        new_panel = await self._build_sample_panel(
            sample, stats, self._tools_count, min_proteins, min_idents
        )
        if self._panels_list is not None:
    ...
    def _thresholds(self) -> tuple[int, int]:
        try:
            min_proteins = int(self._min_proteins_field.value or 30)
        except (ValueError, AttributeError):
            min_proteins = 30
        try:
            min_idents = int(self._min_idents_field.value or 1000)
        except (ValueError, AttributeError):
            min_idents = 1000
    ...
    async def _build_sample_panel(
        ...
        header_content = self._build_sample_header(
            sample, stats, tools_count, min_proteins, min_idents
        )
        body_content = await self._build_sample_body(sample, stats, min_idents)

        return ft.ExpansionPanel(
            header=ft.ListTile(title=header_content),
            content=ft.Container(
    ...
    def _build_sample_header(
        ...
        has_ident_files = if_count > 0
        expected_if = tools_count * sf_count if sf_count > 0 else 0
        ident_files_ok = (expected_if == 0) or (if_count == expected_if)
        idents_ok = idents >= min_idents
        proteins_ok = (proteins == 0) or (proteins >= min_proteins)

        # Status marker
        if not has_spectra or (has_spectra and not has_ident_files):
            marker_icon = ft.Icons.ERROR_OUTLINE_OUTLINED
            marker_color = ft.Colors.RED_600
        elif (
            has_spectra
            and has_ident_files
            and ident_files_ok
            ...
            marker_icon = ft.Icons.CHECK_CIRCLE_OUTLINE_OUTLINED
            marker_color = ft.Colors.GREEN_600
        else:
            marker_icon = ft.Icons.WARNING_AMBER_OUTLINED
            marker_color = ft.Colors.AMBER_600
        ...
        controls += [
            ...
            ft.Text(
                sample.subset_name or "No group",
            ...
            ft.Text(
                f"Idents: {idents}",
                size=11,
                color=ft.Colors.RED_700 if not idents_ok and has_ident_files else ft.Colors.GREY_800,
            ),
        ...
        if empty_if > 0:
            controls.append(
                ft.Icon(
                    ...
                    size=14,
                    tooltip=f"{empty_if} identification file(s) have zero identifications",
                )
            )
    ...
    async def _build_sample_body(
                ...
                for ident_file in sf.get('ident_files', []):
                    ...
                    ident_row = ft.Container(
                        content=ft.Row([
                            ...
                            ft.Text(
                                ident_file.get('tool_name', '?'),
                                size=12,
                                weight=ft.FontWeight.W_500,
                            ),
                            ft.Text(
                                Path(ident_file.get('file_path', '')).name,
                                size=12,
                                color=ft.Colors.GREY_700,
                            ),
                            ft.Text(
        ...
        body_controls.append(
            ft.TextButton(
                ...
                on_click=lambda e, s=sample:
                    self.page.run_task(self._add_spectra_file, s)
                    if self.page else None,
            )
        )
        ...
        left_buttons = ft.Row([
            ft.ElevatedButton(
                ...
                icon=ft.Icons.BOLT,
                on_click=lambda e, s=sample:
                    self.page.run_task(self._action_calculate_ions, s)
                    if self.page else None,
            ),
            ft.ElevatedButton(
                ...
                on_click=lambda e, s=sample:
                    self.page.run_task(self._action_select_preferred, s)
                    if self.page else None,
            ),
            ft.ElevatedButton(
                ...
                icon=ft.Icons.LINK,
                on_click=lambda e, s=sample:
                    self.page.run_task(self._action_match_proteins, s)
                    if self.page else None,
            ),
            ft.ElevatedButton(
                ...
                on_click=lambda e, s=sample:
                    self.page.run_task(self._action_protein_identifications, s)
                    if self.page else None,
            ),
            ft.ElevatedButton(
                content=ft.Text("LFQ"),
                ...
                on_click=lambda e, s=sample:
                    self.page.run_task(self._action_lfq, s)
                    if self.page else None,
            ),
        ], spacing=6, wrap=True)
        ...
        right_buttons = ft.Row([
            ft.ElevatedButton(
                ...
                on_click=lambda e, s=sample:
                    self.page.run_task(self._show_edit_dialog, s)
                    if self.page else None,
            ),
            ft.ElevatedButton(
                content=ft.Row([
                    ft.Icon(
                        ft.Icons.FLAG if sample.outlier else ft.Icons.FLAG_OUTLINED,
                        size=16,
                ...
                on_click=lambda e, s=sample:
                    self.page.run_task(self._toggle_outlier, s)
                    if self.page else None,
            ),
            ft.ElevatedButton(
                content=ft.Row([
                ...
                on_click=lambda e, s=sample:
                    self.page.run_task(self._delete_sample, s)
                    if self.page else None,
            ),
        ], spacing=6)
    ...
    async def _show_tool_picker(self, sf_id: int, sample: Sample, tools):
        ...
        tool_dropdown = ft.Dropdown(
            label="Select Tool", options=options, value=str(tools[0].id), width=300
        )
        ...
        dlg = ft.AlertDialog(
            ...
            actions=[
                ...
                ft.ElevatedButton(
                    "Select",
                    on_click=lambda e: self.page.run_task(on_confirm, e)
                ),
            ],
        )
    ...
    async def _do_add_identification_file(self, sf_id: int, sample: Sample, tool_id: int):
        ...
        dialog = ImportSingleDialog(
            project=self.project,
            page=self.page,
            import_type="identifications",
            tool_id=tool_id,
            on_import_callback=handlers.import_identification_files,
            fixed_sample_name=sample.name,
            fixed_spectra_file_id=sf_id,
            lock_group=True,
        )
        await dialog.show()
    ...
    async def _rebuild_panels_from_cache(self):
        """Rebuild panels list from current _samples + cache (no DB recalc)."""
        all_cached = await self.project.get_all_cached_sample_stats()
        min_proteins, min_idents = self._thresholds()
        ...
        for idx, sample in enumerate(self._samples):
            stats = all_cached.get(sample.id) or self._empty_stats()
            panel = await self._build_sample_panel(
                sample, stats, self._tools_count, min_proteins, min_idents
            )
    ...
    # ------------------------------------------------------------------
    # Calculation actions — all use refresh_single_panel
    # ------------------------------------------------------------------
    ...
    async def _action_select_preferred(self, sample: Sample):
        ...
        if not tool_settings:
            self.show_warning("Configure tool settings in the Peptides tab first.")
            return
    ...
    async def _action_lfq(self, sample: Sample):
        ...
        if state is None:
            self.show_warning("Open Proteins tab first to configure LFQ settings")
            return
    ...
    def _get_protein_detection_params(self) -> tuple[int, int]:
        if self.page and hasattr(self.page, 'proteins_tab'):
            ...
            if ds:
                try:
                    return (
                        int(ds.min_peptides_field.value),
                        int(ds.min_unique_field.value),
                    )
                except (ValueError, AttributeError):
                    pass
        return 2, 1
