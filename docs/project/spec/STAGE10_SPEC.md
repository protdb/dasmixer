...
> **Вопрос 3:** Точнее про conditionдля substitution-ветки: "identification соответствует ограничениям" — имеется в виду условия фильтрации `get_identifications()` (max_ppm, min_score, min_ion_intensity_coverage, min_peptide_length, max_peptide_length)? Или только некоторые из них? Уточни критерий.
